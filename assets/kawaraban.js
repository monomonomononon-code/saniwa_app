(function () {
  ...
  const GUIDE_ITEMS = [
    ...
    },
    {
      ...
      whatItDoes: "部屋割りで作った部屋を3D空間に立体表示する機能です。建築編集モードでは、部屋を「棟」としてまとめたり、廊下・縁側・屋根・門・塀・庭・池・畑・馬小屋・太鼓橋・石・家具(箪笥・ちゃぶ台・布団・ソファ)などを追加・移動・回転・サイズ変更したりできます。「植栽」タブの桜・松・竹・低木・花・樹木は季節に合わせて色や花が変わり、朝・昼・夕・夜の時間帯表示と、春(夏・秋・冬は準備中)の季節表示を別々に切り替えられます。春は花びらが舞います。",
      howToUse: "部屋割りのメニューから「見取り図（3D）」を選びます。「建築編集」で編集モードに入り、パーツ追加のパネルからカテゴリごとに選んで配置します。右上の「朝昼夕夜」で時間帯、「春夏秋冬」で季節を切り替えます。",
