(function () {
  ...
  const GUIDE_ITEMS = [
    ...
    },
    {
      ...
      whatItDoes: "部屋割りで作った部屋を3D空間に立体表示する機能です。建築編集モードでは、部屋を「棟」としてまとめたり、廊下・縁側・屋根・門・塀・庭・池・畑・馬小屋・太鼓橋・石・花びらの絨毯・家具(箪笥・ちゃぶ台・布団・ソファ)などを追加・移動・回転・サイズ変更したりできます。「植栽」タブの桜・松・竹・低木・花・樹木は季節に合わせて姿が変わり、朝・昼・夕・夜の時間帯表示と、春・夏・秋・冬の季節表示を別々に切り替えられます。桜は幹と枝ぶりはそのままに、春は花、夏は緑の葉、秋は紅葉、冬は葉に雪が積もった姿になります。春は花びらが舞い、地面にも散ります。",
      howToUse: "部屋割りのメニューから「見取り図（3D）」を選びます。「建築編集」で編集モードに入り、パーツ追加のパネルからカテゴリごとに選んで配置します。位置合わせ用のグリッドは編集中だけ表示されます。右上の「朝昼夕夜」で時間帯、「春夏秋冬」で季節を切り替えます。池・桜・石・樹木は、選んだときに出る選択欄で見た目を変えられます。池は「丸い池」と「流れの池」、桜は「丸い木」「横に広い木」「背の高い木」、石は「庭石」と「飛び石」から選べます。「部屋名を消す」で部屋名だけを隠せます(刀剣男士名は残ります)。「スクショ用表示」にすると、部屋名・刀剣男士名を含む全てのラベルと画面上のボタン類が消え、3D景観だけになります(画面左上の「表示を戻す」でいつでも解除できます)。",
      savedData: "棟の情報、部屋ごとの3D上の位置・回転、建築パーツ・家具・植栽の位置とサイズ、時間帯と季節の設定、部屋名ラベルの表示設定",
      notes: "端末の性能によっては、配置するパーツや家具が多いほど動作が重くなる場合があります。「スクショ用表示」は撮影用の一時的な表示切替で、画面を再読み込みすると元に戻ります。",
