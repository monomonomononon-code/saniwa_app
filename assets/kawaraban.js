(function () {
  ...
  const GUIDE_ITEMS = [
    ...
    },
    {
      ...
      whatItDoes: "部屋割りで作った部屋を3D空間に立体表示する機能です。建築編集モードでは、部屋を「棟」としてまとめたり、廊下・縁側・屋根・門・塀・庭・池・畑・馬小屋・樹木・家具(箪笥・ちゃぶ台・布団・ソファ)などを追加・移動・回転・サイズ変更したりできます。朝・昼・夕・夜の時間帯表示も切り替えられます。",
      howToUse: "部屋割りのメニューから「見取り図（3D）」を選びます。「建築編集」で編集モードに入り、パーツ追加のパネルからカテゴリごとに選んで配置します。",
      savedData: "棟の情報、部屋ごとの3D上の位置・回転、建築パーツ・家具の位置とサイズ、時間帯の設定",
