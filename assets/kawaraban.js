(function () {
  ...
  const GUIDE_ITEMS = [
    ...
    },
    {
      ...
      howToUse: "部屋割りのメニューから「見取り図（3D）」を選びます。「建築編集」で編集モードに入り、パーツ追加のパネルからカテゴリごとに選んで配置します。右上の「朝昼夕夜」で時間帯、「春夏秋冬」で季節を切り替えます。池・桜・石・樹木は、選んだときに出る選択欄で見た目を変えられます。池は「丸い池」と「流れの池」、桜は「丸い木」「横に広い木」「背の高い木」、石は「庭石」と「飛び石」から選べます。",
