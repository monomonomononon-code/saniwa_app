(function(){
  ...
  let sharedCharacters = SHARED_CHAR_NAMES.map((n, i) => ({
    id: "c" + i, name: n, swordType: "", level: "", activationDate: "", unit: "", isKiwame: false
  }));
    ...
    if (data.type === "character_update" && data.character) {
      ...
      if (idx === -1) {
        sharedCharacters.push(Object.assign(
          { id: data.character.id, name: data.character.name, swordType: "", level: "", activationDate: "", unit: "", isKiwame: false },
          data.character
        ));
      } else {
  ...
  function showView(id) {
    ...
    if (id === "journal-view" && !journalLoaded) {
      journalSub.iframe.src = "pages/journal.html?v=caret-follow-2";
      journalLoaded = true;
    }
    if (id === "backup-view") {
    ...
    if (id === "honmaru3d-view") {
      ...
      if (!honmaru3dLoaded) {
        honmaru3dSub.iframe.src = "pages/honmaru3d.html?v=drag-state-1";
    ...
    if (id === "expcalc-view" && !expcalcLoaded) {
      expcalcSub.iframe.src = "pages/expcalc.html";
      expcalcLoaded = true;
    }
  }
