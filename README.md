...
- `pages/honmaru3d.html` — 本丸建築エディタ(3D俯瞰図)。部屋タイプ・建築パーツは同ファイル冒頭の `TEMPLATE_META` / `PART_DEFS` / `PART_RENDERERS` で定義。時間帯は `TIME_PRESETS`、季節は `SEASON_PRESETS`(春夏秋冬を実装済み。各季節は `palette`(色の差分)・`foliage`(樹冠を花にするか葉にするか、雪、地面の花びら、低木の花)・`effects`(舞い散る演出)を持ち、書いていないキーは春の値で補われる)。桜は幹・枝・房の位置を共通にしたまま、`foliage.canopy` で花と葉を差し替え、`foliage.snow` で雪を乗せる。パーツごとの見た目オプション(池のかたち・桜の樹形・石の置き方・樹形)は `PART_PROPS` に定義すると情報パネルの選択欄が自動で増え、値は `item.props` に入る。池や岩組みの「四角く見せない輪郭」は `blobPoints` / `blobShape` で作る。桜の花房・樹木や低木の葉は「交差板 + テクスチャ」方式。`blossomMaterial`(花)と `leafMaterial(中間色, 濃色, 明色)`(葉)が素材を作り、`scatterFoliage` が房をドーム状に散らし、`foliageGeometry` が1メッシュにまとめる。葉の色は季節パレットの `canopy` / `shrub` 系から取るので、夏は濃い緑、秋は黄〜赤、と `SEASON_PRESETS` の色を変えるだけで差し替わる(冬の落葉は葉のメッシュを作らず枝だけ描けばよい)。桜の樹形は `SAKURA_FORMS` に1行足すと増やせる。地面は起伏なしの平面2枚で、外周が `grassTexture`(草地のタイル)、中心が `earthTexture`(乾いた土。ふちを透明にぼかして草地となじませる)。色は季節パレットの `groundEarth` / `groundGrass` / `pebble` 系から作る。土のエリアは本丸の広がりに合わせて自動で広がる。スナップ用のグリッドは建築編集中だけ表示される。建築・景観パーツの名札は3D空間には常時表示しない(選択時の名称は情報パネル側で表示)。部屋名・刀剣男士名のラベルは `pickables` に積んだ `THREE.Sprite` の `visible` を `updateLabelVisibility` で出し分けており、「部屋名を消す」(`roomLabelsVisible`)は部屋名のみ、「スクショ用表示」(`screenshotMode`、再読み込みで必ず解除)は両方とUI全体を隠す。保存キーは `saniwa-tool.honmaru3d.v1`(version 2、`timeMode` / `seasonMode` / `roomLabelsVisible` を含む)
